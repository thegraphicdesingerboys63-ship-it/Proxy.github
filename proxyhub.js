...
document.addEventListener('keydown',e=>{
  if(e.key==='ArrowUp'&&d.y===0)nd={x:0,y:-1};
  if(e.key==='ArrowDown'&&d.y===0)nd={x:0,y:1};
  if(e.key==='ArrowLeft'&&d.x===0)nd={x:-1,y:0};
  if(e.key==='ArrowRight'&&d.x===0)nd={x:1,y:0};
  if(e.key===' '&&go)restart();
  e.preventDefault();
});
...
function draw(){
  ...
  ctx.fillStyle='#f87171';ctx.shadowColor='#f87171';ctx.fillRect(p2.x,p2.y,PW,PH);
...
document.getElementById('backBtn').addEventListener('click', () => {
  if (historyIdx > 0) { historyIdx--; const u = history[historyIdx]; urlBar.value = u; frame.src = '/fetch?url=' + encodeURIComponent(u); }
});
document.getElementById('fwdBtn').addEventListener('click', () => {
  if (historyIdx < history.length - 1) { historyIdx++; const u = history[historyIdx]; urlBar.value = u; frame.src = '/fetch?url=' + encodeURIComponent(u); }
});
document.getElementById('reloadBtn').addEventListener('click', () => { if (frame.src) frame.src = frame.src; });
